...
class httplog extends Controller{
    private $sql;
    function __construct(){
		...
		public function init() {
			...
			$results= $db->query("select * from httplog order by id desc limit 10");	
			$data= array();
			$i = 0; 
			$lnum = 0;
			$fnum = 0;
    ...
    public function clean() {
			...
			if ($db)
			{
				$results= $db->query("delete from httplog");	
			}
			else
			{
				show_json($L['data_error'],false);
			...
			$results= $db->query("update sqlite_sequence set seq = 0 where name = 'httplog'");
			$results= $db->query("select count(*) as count from httplog");
...
if($filterweb != " "){
	...
	$afterfilter= $db->query("select count(*) as count from httplog" . $where2 . $filterweb);
	$aftercount=0;
	while ($res= $afterfilter->fetchArray(1))
	{
		...
		//数据库
		$results= $db->query("select * from httplog"  . $where .  $filterweb . "order by id desc limit 10"); //倒序
		$data= array();
		$i = 0; 
		while ($res= $results->fetchArray(1))
		{
	...
	if($data == array()){
		...
		//数据库（正序）
$results1= $db->query("select * from  httplog"  . $where .  $filterweb . "order by id asc limit " . $limitcount);  //正序
$data= array();
$ascdata = array();
$k = 0; 
while ($res= $results1->fetchArray(1))
{
	...
	array_push($ascdata, $res);
		...
		for($k = count($ascdata)-1;$k>=0;$k--){
			...
			$where=" where id > " . $lnum;
					//数据库（正序）
$results1= $db->query("select * from httplog"  . $where .  $filterweb . "order by id asc limit 10");  //正序
$data= array();
$ascdata = array();
$k = 0; 
	while ($res= $results1->fetchArray(1))
...
//拼接总数和id号
			$info= array('count' => $count,'fnum'=> $fnum,'lnum' => $lnum);
			// $info= array('count' => $count,'fnum'=> $fnum,'lnum' => $lnum,'prevlnum' => $prevlnum);  注释
    ...
    public function export() {
			...
			if ($db)
			{
				$results= $db->query("select * from httplog");	
			}
			else
			{
				return;
			...
			system("/sbin/sqlite3 -header -csv /var/spool/antivirus/log.db 'select * from httplog;' > /tmp/weblog.txt");
			
			header('Content-type: text/csv; charset=utf-8');
			header("Content-Disposition: attachment; filename=weblog.txt");
			
			$fp = fopen("/tmp/weblog.txt", "rb");
			fseek($fp, $start);
			while (!feof($fp)) {
