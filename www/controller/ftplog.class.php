...
class ftplog extends Controller{
    private $sql;
    function __construct(){
		...
		public function init() {
			...
			if ($db)
			{
				$results= $db->query("select count(*) as count from ftplog");
			}
			else
			{
			...
			$results= $db->query("select * from ftplog order by id desc limit 10");	
			$data= array();
			$i = 0; 
			$lnum = 0;
			$fnum = 0;
    ...
    public function clean() {
			...
			if ($db)
			{
				$results= $db->query("delete from ftplog");	
			}
			else
			{
				show_json($L['data_error'],false);
			...
			$results= $db->query("update sqlite_sequence set seq = 0 where name = 'ftplog'");	
...
$afterfilter= $db->query("select count(*) as count from ftplog" . $where2 . $filterweb);
$aftercount=0;
while ($res= $afterfilter->fetchArray(1))
{
	...
	//数据库
	$results= $db->query("select * from ftplog"  . $where .  $filterweb . "order by id desc limit 10"); //倒序
	$data= array();
	$i = 0; 
	while ($res= $results->fetchArray(1))
	{
	...
	if($data == array()){
		...
		//数据库（正序）
$results1= $db->query("select * from  ftplog"  . $where .  $filterweb . "order by id asc limit " . $limitcount);  //正序
$data= array();
$ascdata = array();
$k = 0; 
while ($res= $results1->fetchArray(1))
{
	...
	array_push($ascdata, $res);
		...
		$where=" where id > " . $lnum;
				//数据库（正序）
$results1= $db->query("select * from  ftplog"  . $where .  $filterweb . "order by id asc limit 10");  //正序
$data= array();
$ascdata = array();
$k = 0; 
while ($res= $results1->fetchArray(1))
...
//拼接总数和id号
    ...
    public function export() {
			...
			if ($db)
			{
				$results= $db->query("select * from ftplog");	
			}
			else
			{
				return;
			...
			system("/sbin/sqlite3 -header -csv /var/spool/antivirus/log.db 'select * from ftplog;' > /tmp/ftplog.txt");
			
			header('Content-type: text/csv; charset=utf-8');
			header("Content-Disposition: attachment; filename=ftplog.txt");
			
			$fp = fopen("/tmp/ftplog.txt", "rb");
			fseek($fp, $start);
			while (!feof($fp)) {
