...
class explorer extends Controller{
    ...
    public function treeList(){//树结构
        ...
        if (isset($this->in['type']) && $this->in['type']=='check'){
            $this->_check_usb();
        }
        if (isset($this->in['type']) && $this->in['type']=='umount'){
            $this->_umount_usb($this->in['usbid']);
        }
    ...
    private function _check_usb(){
    	$arr = scandir('/mnt/usbox/');
    	$usbcount = count($arr)-2;
    	$data = array(
    		'usbcount' => $usbcount
    	);
    	show_json($data);
    ...
    private function _tree_init($app){
        ...
        $tree_data = array(
            ...
            array('name'=>$this->L['secros_name_desc'],'children'=>$public,'menuType'=>"menuTreeRoot",
                'iconSkin'=>"lib",'open'=>true,'this_path'=> '*usbox*','isParent'=>$public_isparent)
        );
        show_json($tree_data);
    }
    ...
    public function pathDelete(){
        ...
        foreach ($list as $val) {
            ...
            //$info = 'rm -rf '.$path_this;
            //show_json($info,false);
            //if (@rename($path_this,$filename)) {
            //system($info,$return_var);
           //if (@unlink($path_this)) {
           //     $success++;
           // }else{
           //     $error++;
...
show_json('没有权限',false);
    ...
    public function _make_file_proxy($file_path){
        ...
        //文件对外界公开的地址;有效期在user_setting.php中设定；末尾追加文件名为了secros远程下载
    ...
    public function officeView(){
        ...
        if (strstr($host,'10.10.') ||
            ...
            ', <a href="http://secros.com/help.html#office" target="_blank">'.
            $this->L['more'].'>></a>';
    ...
    private function path($dir,$list_file=true,$check_children=false){
        ...
        foreach ($list['filelist'] as $key => $val) {
            /*if (!in_array($val['ext'],$ex_name)) continue;
            if ($val['ext'] == 'oexe'){
